...
/* ⭐ STABLE AND FIXED INPUT STYLING ⭐ */

/* 1. Target the Selectbox (Geographic Region) */
...
/* 2. Target the Text Inputs (Hashtags / URLs) + Force Border/Shadow Deletion */
...
/* 3. Target the Date Inputs (From / To) + Structural Clean */
...
/* 4. Formatting fonts and arrow colors correctly */
...
/* Active focus fallback override across the field blocks */
...
/* 📏 SHRINK THE "＋ ADD" BUTTON SQUARE FRAME */
...
with st.sidebar:
    ...
    if st.button(" Home Overview", use_container_width=True):
        ...
        st.rerun()
    if st.button(" Campaign Config", use_container_width=True):
...
# ─── 2. CONFIGURATION MODE ───────────────────────────────────────────────────
    ...
    with col_config:
        ...
        region = st.selectbox("", ["Worldwide","United States","Europe","Israel","Asia"], label_visibility="collapsed")
        ...
        with c1:
            ...
            start_date = st.date_input("", datetime.date.today() - datetime.timedelta(days=2), label_visibility="collapsed")
        with c2:
            ...
            end_date = st.date_input("", datetime.date.today(), label_visibility="collapsed")
        ...
        max_posts = st.slider("", 500, 10000, 2000, step=500, label_visibility="collapsed")
...
# ─── 3. DASHBOARD MODE ───────────────────────────────────────────────────────
    ...
    st.markdown("<h2 style='color:#f472b6'>Campaign Integrity & Brand Protection Analytics</h2>", unsafe_allow_html=True)
